...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        static void Blur2()
        {
            if (!instances.ContainsKey(selectedId))
            {
                return;
            }
            var block = instances[selectedId].blurBlock;    // get blur block state
            instances[selectedId].blurBlock = false;        // reset instalce block state
            if (block)
            {
                return;
            }
            instances[selectedId].DeactivateInputField();
        }
